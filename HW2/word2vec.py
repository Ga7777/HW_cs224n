...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.dot(yhat.reshape([-1,1]), vc.reshape([1,-1]))
...
def negSamplingLossAndGradient(
    ...
    U, vc, o = outsideVectors, centerWordVec, outsideWordIdx
    score = np.dot(U[indices], vc)
    score[1:] *= -1 # 除了Uo在sigmoid前为正值，其余Uk前都为负值
    score = sigmoid(score)
    loss = -np.sum(np.log(score))

    score = 1 - score
    score[0] *= -1
    gradCenterVec = np.dot(score.reshape([1, -1]), U[indices]).squeeze()
    # squeeze()从数组的形状中删除单维度条目，即把shape中为1的维度去掉

    gradOutsideVecs = np.zeros_like(U)
    _gradOutsideVecs = np.dot(score.reshape([-1, 1]), vc.reshape([1, -1]))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    vc = centerWordVectors[center_word_indx]

    for w in outsideWords:
        _loss, _gradCenterVec, _gradOutsideVectors = word2vecLossAndGradient(vc, word2Ind[w], outsideVectors,dataset)
        loss += _loss
...
# == == Gradient
# check for skip - gram with negSamplingLossAndGradient == ==
# Gradient check passed!
